//Challenges:
//play 3 different sounds
//animation to show winner
//randomized velocity of ball
//additional function: randomized length of paddle to decrease the possibililty of hitting the ball
...
function preload() {
  soundFormats('mp3');
  pong = loadSound('project-folder/pong.mp3');
  offtable = loadSound('project-folder/offtable.mp3');
...
if (score.player1 == 5 ) {
    ...
    score.player2 = score.player2;
  	...
  	star(0, 0, 10, 20, 5); 
 		pop();
 		textAlign(CENTER);
  	text('WINNER', width*0.25, height*0.2);
    // play gamesound
    if(!gameover.played) {
    ...
    score.player2 = score.player2;
  	...
  	star(0, 0, 10, 20, 5); 
 		pop();
 		textAlign(CENTER);
  	text('WINNER', width*0.75, height*0.2);
    // play gamesound
    if(!gameover.played) {
    gameover.setVolume(0.1);
    gameover.play();
  ...
  if (GAMESTATE == 'START') {
    ...
    fill (50);
    rect (width / 4, height / 2.2, width / 2, height / 13);
    fill (255);
    textAlign(CENTER);
  ...
  ellipse(ball.x, ball.y, ball.size);
    ...
    // play point sound
    if(!offtable.played) {
    offtable.setVolume(0.1);
    offtable.play();
    offtable.played = true;
		}
    fill (50);
    rect (width / 4, height / 2.2, width / 2, height / 13);
    fill (255);
    ...
    fill (50);
    rect (width / 6, height / 2.2, width / 1.5, height / 13);
    fill (255);
    textAlign(CENTER);
    text('GAME OVER! Press the play button to restart', width / 2, height / 2);
  }
}
...
function paddleSize(){
  paddle1.length = random (50, 100);
  paddle2.length = random (50, 100);
  
}
function initializePositions() {
  ball.x = width / 2;
  ball.y = random (100, 350);
  ball.xvelocity = random(2, 10); 
  ball.yvelocity = 2;
